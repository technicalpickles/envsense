...
fn demonstrate_dependency_injection_benefits() {
    ...
    let all_tty_snapshot = EnvSnapshot::for_testing(
        HashMap::new(),
        TtyDetector::mock_all_tty()
    );
    ...
    let no_tty_snapshot = EnvSnapshot::for_testing(
        HashMap::new(),
        TtyDetector::mock_no_tty()
    );
    ...
    let piped_snapshot = EnvSnapshot::for_testing(
        HashMap::new(),
        TtyDetector::mock_piped_io()
    );
...
fn compare_old_vs_new_approach() {
    ...
    for (scenario_name, stdin, stdout, stderr) in scenarios {
        ...
        println!("  TTY config: stdin={}, stdout={}, stderr={}", stdin, stdout, stderr);
        println!("  is_interactive: {}", snapshot.is_tty_stdin() && snapshot.is_tty_stdout());
        println!();
