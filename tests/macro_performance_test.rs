...
fn benchmark_macro_merging_performance() {
    ...
    let detections: Vec<Detection> = (0..1000)
        .map(|i| Detection {
            contexts_add: vec![
                if i % 2 == 0 { "agent".to_string() } else { "ide".to_string() },
                if i % 3 == 0 { "ci".to_string() } else { "container".to_string() },
            ],
            traits_patch: HashMap::from([
                ("is_interactive".to_string(), serde_json::Value::Bool(i % 2 == 0)),
                ("is_tty_stdout".to_string(), serde_json::Value::Bool(i % 3 == 0)),
                ("supports_hyperlinks".to_string(), serde_json::Value::Bool(i % 4 == 0)),
            ]),
            facets_patch: HashMap::from([
                ("agent_id".to_string(), serde_json::Value::String(format!("agent-{}", i))),
                ("ide_id".to_string(), serde_json::Value::String(format!("ide-{}", i))),
            ]),
            evidence: vec![
    ...
    assert!(duration.as_micros() < 10000, "Merging took {}μs, expected < 10000μs", duration.as_micros());
    
    // Verify the merging worked correctly
    assert!(envsense.contexts.agent || envsense.contexts.ide);
    assert!(envsense.traits.is_interactive || !envsense.traits.is_interactive);
    assert!(envsense.evidence.len() > 0);
    
    println!("Macro merging performance: {}μs for 1000 detections", duration.as_micros());
}
...
fn benchmark_macro_vs_manual_approach() {
    ...
    let detections = vec![
        Detection {
            ...
            facets_patch: HashMap::from([
                ("agent_id".to_string(), serde_json::Value::String("cursor".to_string())),
                ("ide_id".to_string(), serde_json::Value::String("cursor".to_string())),
            ]),
            evidence: vec![
                serde_json::to_value(Evidence {
                    signal: Signal::Env,
    ...
    println!("Macro-based merging completed in {}μs", macro_duration.as_micros());
