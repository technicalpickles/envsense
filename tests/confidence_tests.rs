use envsense::detectors::confidence::{HIGH, LOW, MEDIUM, TERMINAL};
...
fn test_confidence_constants() {
    ...
    // These are compile-time constants, so we can verify them statically
    const _: () = {
...
fn test_detection_confidence_values() {
    ...
    for evidence in &result.evidence {
        match evidence.signal {
            Signal::Env => {
                if evidence.value.is_some() {
                    assert_eq!(
                        ...
                        "Direct env var should have HIGH confidence: {}",
                        evidence.key
                    );
                } else {
                    ...
                    assert!(
                        ...
                        "Env presence should have MEDIUM or HIGH confidence: {} = {}",
                        evidence.key,
                        evidence.confidence
                    );
                }
            ...
            Signal::Tty => {
                assert_eq!(
                    evidence.confidence, TERMINAL,
                    "TTY detection should have TERMINAL confidence: {}",
                    evidence.key
                );
            }
            _ => {
                assert!(
                    evidence.confidence >= LOW && evidence.confidence <= HIGH,
...
fn test_confidence_override() {
    ...
    let evidence = Evidence::env_var("TEST_VAR", "test_value").with_confidence(LOW);
    ...
    for evidence in &result.evidence {
        match evidence.signal {
            ...
            _ => {
                ...
                assert!(
                    evidence.confidence >= LOW && evidence.confidence <= HIGH,
                    "Signal {:?} should have confidence in [LOW, HIGH] range",
                    evidence.signal
                );
            }
        }
