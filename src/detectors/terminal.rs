...
use crate::traits::terminal::{TerminalTraits, ColorLevel};
use serde_json::json;
...
impl Detector for TerminalDetector {
    ...
    fn detect(&self, snap: &EnvSnapshot) -> Detection {
        ...
        detection.traits_patch.insert("is_interactive".to_string(), json!(terminal_traits.is_interactive));
        ...
        detection.traits_patch.insert("supports_hyperlinks".to_string(), json!(terminal_traits.supports_hyperlinks));
        ...
        detection.traits_patch.insert("color_level".to_string(), json!(color_level_str));
...
mod tests {
    ...
    fn create_env_snapshot(is_tty_stdin: bool, is_tty_stdout: bool, is_tty_stderr: bool) -> EnvSnapshot {
        EnvSnapshot {
    ...
    fn detects_terminal_traits() {
        ...
        assert_eq!(detection.traits_patch.get("is_tty_stdin").unwrap(), &json!(true));
        assert_eq!(detection.traits_patch.get("is_tty_stdout").unwrap(), &json!(true));
        assert_eq!(detection.traits_patch.get("is_tty_stderr").unwrap(), &json!(false));
        assert_eq!(detection.traits_patch.get("is_piped_stdin").unwrap(), &json!(false));
        assert_eq!(detection.traits_patch.get("is_piped_stdout").unwrap(), &json!(false));
    ...
    fn detects_piped_io() {
        ...
        assert_eq!(detection.traits_patch.get("is_tty_stdin").unwrap(), &json!(false));
        assert_eq!(detection.traits_patch.get("is_tty_stdout").unwrap(), &json!(false));
        assert_eq!(detection.traits_patch.get("is_tty_stderr").unwrap(), &json!(false));
        assert_eq!(detection.traits_patch.get("is_piped_stdin").unwrap(), &json!(true));
        assert_eq!(detection.traits_patch.get("is_piped_stdout").unwrap(), &json!(true));
    }
}